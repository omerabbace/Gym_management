...
if(isset($_POST['change'])){
        header("Location:update_trainer.php");
}
...
<?php
$sql_assign= "SELECT * from assign_trainer where trainee_id = $id_trainee ";
// echo $sql_assign;
$res_assign = mysqli_query($conn,$sql_assign);
if(mysqli_num_rows($res_assign)>0){
        while($row3=mysqli_fetch_assoc($res_assign)){
                $trainerId =$row3['trainer_id'];
        }
        ?>
...
account on account.person_id=trainer.person_id join assign_trainer on
assign_trainer.trainer_id=trainer.trainer_id where assign_trainer.trainer_id=$trainerId ";
// echo $sql;
$result = mysqli_query($conn,$sql);
...
?>
  <tbody>
    <tr>
      ...
      <td><input type="submit" name="change" value="Change" style="margin-left: 20px; background-color:red;
